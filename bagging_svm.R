...
training <- seq(from =1, to= 500, by=1)
train <- Data[training, ]
...
SVMmodel <- svm(train[,response]~ ., data = train[,-1], type = 'C')
# look at summary
summary(SVMmodel)
...
fitted.results<-predict(SVMmodel,test[,-25],type = 'response')
...
for (b in 1:B)
{
  ...
  SVMmodel <- svm(train[sampledata,][,response]~ ., data = train[sampledata,][,-1], type = 'C')
  summary(SVMmodel)
  ...
  fitted.results <- data.frame(predict(SVMmodel,test[,-1],type = 'response') )
...
rowSums(x1[,-1])
finalresult <- ifelse(rowSums(x1[,-1])>2, 1,0)
y_test <- test[,25]
summary(y_test)
summary(finalresult)
